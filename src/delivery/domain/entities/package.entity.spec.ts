...
describe('Package Entity', () => {
  it('debería crear un paquete con estado pending por defecto', () => {
    const address = new Address('Calle Principal 123', 'La Paz', -16.5, -68.1);
    const pkg = new Package('PKG001', 'PATIENT001', new Date('2024-01-20'), address, 'ROUTE001');
  ...
  it('debería convertir el paquete a objeto de persistencia correctamente', () => {
    ...
    const pkg = new Package('PKG004', 'PATIENT004', deliveryDate, address, 'ROUTE001');
  ...
  it('debería crear un paquete mediante el método estático create', () => {
    const address = new Address('Avenida Secundaria 456', 'Cochabamba', -17.4, -66.2);
    const deliveryDate = new Date('2024-01-21');

    const pkg = Package.create('PKG005', 'PATIENT005', deliveryDate, address);
