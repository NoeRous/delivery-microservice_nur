...
describe('Pact Consumer tests (delivery-consumer -> delivery-provider)', () => {
  describe('when a request to create a package is made', () => {
    ...
    afterEach(async () => {
      try {
        ...
        console.error('Pact verification failed!', err && err.message ? err.message : err);
    ...
    it('creates a package and returns id (mocked)', async () => {
      ...
      try {
        const res = await axios.post('http://localhost:1234/packages', reqBody, { headers: { 'Content-Type': 'application/json' } });
        expect(res.status).toBe(201);
        expect(res.data).toHaveProperty('id');
      } catch (err: any) {
        console.error('Axios error on create-package call:', err && err.message ? err.message : err);
        throw err;
      }
    });
  });
  ...
  describe('when a request to assign a package to a dealer is made', () => {
    ...
    it('assigns a package to a dealer (mocked)', async () => {
      ...
      try {
        const res = await axios.post('http://localhost:1235/packages/assign', reqBody, { headers: { 'Content-Type': 'application/json' } });
        expect(res.status).toBe(200);
        expect(res.data).toHaveProperty('success');
      } catch (err: any) {
        console.error('Axios error on assign-package call:', err && err.message ? err.message : err);
        throw err;
      }
    });
  });
